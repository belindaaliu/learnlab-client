      ...
      <header className="bg-white h-20 border-b border-gray-200 shadow-sm flex items-center px-6 justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="relative group">
            <Link to="/student/learning" className="text-sm font-medium text-gray-700 hover:text-primary transition">
              My Learning
            </Link>
            <div className="absolute left-0 top-full w-80 bg-white shadow-lg border rounded-lg z-50
              ...
              <div className="p-4 space-y-4">
                {purchasedCourses.slice(0, 3).map(course => (
                  <div key={course.id} className="flex gap-3">
                    <img src={course.thumbnail_url} className="w-16 h-10 object-cover rounded" />
                    <div className="flex-1">
                      <p className="text-sm font-medium line-clamp-2">{course.title}</p>
                      <Link to={`/course/${course.id}`} className="text-xs text-primary font-semibold hover:underline">
                        Start learning
                      </Link>
                    </div>
                  </div>
              ...
              <div className="border-t p-3">
                <Link to="/student/learning" className="block text-center text-sm font-semibold text-primary hover:underline">
                  Go to My Learning
                </Link>
              </div>
            </div>
          ...
          {/* Wishlist dropdown */}
          <div className="relative group">
            <Link to="/student/learning?tab=wishlist" className="text-sm font-medium text-gray-700 hover:text-primary transition">
              Wishlist
            </Link>
            <div className="absolute left-0 top-full w-80 bg-white shadow-lg border rounded-lg z-50
                            opacity-0 group-hover:opacity-100 pointer-events-none
              ...
              <div className="p-4 space-y-4">
                {wishlistCourses.slice(0, 3).map(course => (
                  <div key={course.id} className="flex gap-3">
                    <img src={course.thumbnail_url} alt={course.title} className="w-16 h-10 object-cover rounded" />
                    <div className="flex-1">
                      <p className="text-sm font-medium line-clamp-2">{course.title}</p>
                      <Link to={`/course/${course.id}`} className="text-xs text-primary font-semibold hover:underline">
                        View course
              ...
              <div className="border-t p-3">
                <Link to="/student/learning?tab=wishlist" className="block text-center text-sm font-semibold text-primary hover:underline">
                  Go to Wishlist
                </Link>
              </div>
            </div>
          ...
          <div className="relative group cursor-pointer">
            {realUser?.photo_url ? (
              <img src={realUser.photo_url} alt="User Avatar" className="w-10 h-10 rounded-full object-cover" />
            ) : (
              <div className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center font-semibold">
                {realUser?.first_name?.[0]}{realUser?.last_name?.[0]}
              </div>
            )}

            <div className="absolute right-0 top-full w-64 bg-white shadow-xl rounded-lg z-50
                            opacity-0 group-hover:opacity-100 hover:opacity-100
                            pointer-events-none group-hover:pointer-events-auto transition">
              <div className="px-4 py-3 border-b">
                <p className="font-semibold">{realUser?.first_name} {realUser?.last_name}</p>
              ...
              <div className="py-2">
                ...
                <Link to="/student/learning?tab=wishlist" className="block px-4 py-2 hover:bg-gray-100">Go to Wishlist</Link>
                ...
                <Link to="/student/subscription" className="block px-4 py-2 hover:bg-gray-100">Subscriptions</Link>
                <Link to="/student/payment/history" className="block px-4 py-2 hover:bg-gray-100">Purchase history</Link>
                <div className="border-t my-2"></div>
                <Link to={`/${user.role}/public-profile/${user.id}`} className="block px-4 py-2 hover:bg-gray-100">Public profile</Link>
