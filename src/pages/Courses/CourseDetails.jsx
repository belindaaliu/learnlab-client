...
import { Clock, BookOpen, Star, User, Globe, CheckCircle, PlayCircle, Lock, AlertCircle, Target } from "lucide-react";
          ...
          <div className="lg:w-2/3 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm pt-2">
               ...
               <div className="flex items-center gap-1 text-yellow-400">
                  ...
                  <div className="flex"><Star className="w-4 h-4 fill-current" /><Star className="w-4 h-4 fill-current" /><Star className="w-4 h-4 fill-current" /><Star className="w-4 h-4 fill-current" /></div>
                  <span className="text-blue-300 underline ml-1 cursor-pointer">(120 ratings)</span>
               </div>
               <div className="text-slate-300">
                  Created by <span className="text-purple-300 underline cursor-pointer">{course.Users?.first_name} {course.Users?.last_name}</span>
               </div>
            </div>
            
            <div className="flex items-center gap-4 text-sm text-slate-300 mt-2">
                <span className="flex items-center gap-1"><Clock className="w-4 h-4"/> Last updated 1/2026</span>
               ...
               {[1, 2, 3].map((section, idx) => (
                 <div key={idx} className="border-b border-gray-100 last:border-0">
                   <div className="bg-gray-50 px-4 py-3 font-bold text-gray-800 flex justify-between cursor-pointer hover:bg-gray-100">
                     ...
                     <span className="text-xs text-gray-500 font-normal">3 lectures • 15min</span>
                   </div>
                   <div className="p-4 space-y-3 bg-white">
                      <div className="flex justify-between text-sm text-gray-600 group">
                        <div className="flex items-center gap-3">
                           ...
                           <span className="group-hover:underline cursor-pointer" onClick={() => openPreview(`Lecture ${section}.1`)}>Introduction to Section {section}</span>
                        </div>
                        <div className="flex items-center gap-4">
                            <span className="text-purple-600 font-bold text-xs cursor-pointer hover:text-purple-800 underline" onClick={() => openPreview(`Lecture ${section}.1`)}>Preview</span>
        ...
        <div className="lg:col-span-1 relative">
           <div className="bg-white p-1 shadow-xl border border-gray-200 lg:sticky lg:top-24 -mt-32 lg:mt-0 z-10 w-full lg:w-[340px] ml-auto">
              ...
              <div 
                ...
                onClick={() => openPreview()}
              >
                 <img src={course.thumbnail_url || course.image} alt={course.title} className="w-full h-full object-cover" />
                 ...
                 <div className="absolute bottom-4 left-0 right-0 text-center text-white font-bold text-lg drop-shadow-md">Preview this course</div>
              </div>

              <div className="p-6 space-y-4">
                 <div className="flex items-center gap-3 mb-2">
                    <span className="text-3xl font-bold text-gray-900">${course.price}</span>
                    <span className="text-lg text-gray-400 line-through">${Number(course.price) * 2}</span>
                    <span className="text-sm text-gray-500">50% off</span>
                 </div>

                 <Button fullWidth size="lg" className="h-12 text-lg font-bold">Add to Cart</Button>
                 ...
                 <div className="space-y-2 text-sm text-gray-700 pt-2">
                    ...
                    <div className="flex gap-2 items-center"><Clock className="w-4 h-4"/> 56.5 hours on-demand video</div>
                    <div className="flex gap-2 items-center"><BookOpen className="w-4 h-4"/> 15 articles</div>
                    <div className="flex gap-2 items-center"><Globe className="w-4 h-4"/> Access on mobile and TV</div>
                 </div>
              </div>
           </div>
        </div>
