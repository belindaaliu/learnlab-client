...
import { AlertCircle, CheckCircle, XCircle, HelpCircle, ArrowLeft, BarChart3 } from "lucide-react";
...
const QuizPlayer = ({ lessonId, courseId, onQuizComplete }) => {
  ...
  useEffect(() => {
    ...
    if (prevLessonIdRef.current !== lessonId) {
      console.log("Lesson ID changed from", prevLessonIdRef.current, "to", lessonId);
  ...
  const fetchQuizData = async () => {
    ...
    try {
      ...
      if (response.data.previous_attempt && response.data.previous_attempt.score >= 55) {
        ...
        try {
          ...
          console.log("Could not fetch detailed results, creating basic results");
          setResults({
            score: response.data.previous_attempt.score,
            correct_count: Math.round((response.data.previous_attempt.score / 100) * response.data.total_questions),
      ...
      const initialAnswers = response.data.questions.map(question => ({
        ...
        selected_option_id: question.previous_answer?.selected_option_id || null,
      ...
      if (error.response?.status === 404) {
        setError("Quiz not found. The assessment endpoint might not be implemented yet.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleAnswerSelect = (optionId, questionType) => {
    ...
    const answerIndex = newAnswers.findIndex(a => a.question_id === currentQuestionId);
  ...
  const handleRetakeQuiz = () => {
    ...
    setHasPreviousPass(false);
    setCurrentQuestion(0);
    setAnswers(quizData.questions.map(question => ({
      question_id: question.id,
  ...
  const answeredQuestions = answers.filter(a => 
    a.selected_option_id || (a.answer_text && a.answer_text.trim())
  ).length;
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <p className="text-blue-700 font-medium">
                  You have already passed this quiz on {new Date(results.completed_at || Date.now()).toLocaleDateString()}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="text-center mb-8">
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-600">
                {!isPreviousPass && `Time taken: ${formatTime(results.time_taken || timeSpent)}`}
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-gray-600">{results.total_questions}</div>
          ...
          <div className="text-center">
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm text-gray-500">55% required to pass</span>
              </div>
            ...
            <div className="bg-gray-50 rounded-lg p-6 mb-8">
              ...
              {quizData.instructions && (
                <div className="text-left mb-6">
                  ...
                  <p className="text-gray-700 whitespace-pre-line">{quizData.instructions}</p>
                </div>
              )}
              ...
              {quizData.previous_attempt && !hasPreviousPass && (
                <div className="bg-yellow-50 border border-yellow-200 rounded p-4 mb-6">
                  <p className="text-yellow-800">
                    You previously scored <span className="font-bold">{quizData.previous_attempt.score}%</span> 
                    on {new Date(quizData.previous_attempt.completed_at).toLocaleDateString()}
                  </p>
                </div>
              )}
            </div>
      ...
      <div className="h-full bg-white flex flex-col">
        ...
        <div className="border-b border-gray-200 p-4">
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">{quizData.title}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="max-w-3xl mx-auto">
            <div className="mb-8">
              {reviewMode && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-sm text-blue-800">
                    <strong>Review Mode:</strong> You can review and change your answers before submitting.
                  </p>
                </div>
              )}
              ...
              {question.question_type === 'mcq' || question.question_type === 'truefalse' ? (
                <div className="space-y-3">
                  {question.options.map((option) => (
                    <div
                      ...
                      onClick={() => handleAnswerSelect(option.id, question.question_type)}
                    >
