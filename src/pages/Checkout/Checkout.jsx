...
//   // useEffect(() => {
//   //   // If no state exists (due to hard redirect), check the recovery storage
//   //   if (!checkoutData) {
//   //     const recovery = sessionStorage.getItem("checkout_recovery");
//   //     if (recovery) {
//   //       setCheckoutData(JSON.parse(recovery));
//   //       sessionStorage.removeItem("checkout_recovery"); // Clean up
//   //     }
//   //   }
//   // }, [checkoutData]);
...
const CheckoutPage = () => {
  ...
  // We check location.state first, then fallback to sessionStorage
  const checkoutData = useMemo(() => {
    ...
    if (saved) {
      ...
      // Clean up after reading so it doesn't linger
  ...
  useEffect(() => {
    // If we've checked everywhere and still have no amount, go home
            ...
            <>
              {cartItems.map((item) => (
                <div key={item.id} className="flex justify-between text-sm py-1">
                  <span className="truncate max-w-[150px] text-gray-600 font-medium">{item.title}</span>
                  <span className="font-bold text-gray-900">CA${Number(item.price).toFixed(2)}</span>
                </div>
              ))}
            </>
          )}
